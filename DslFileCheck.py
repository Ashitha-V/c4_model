...
# Class includes all the backend methods related to fetching of Products and Containers names from DSL file.
class DslFileCheck:
    file_name = ''
    file_data = ''
    file_data_list = []
    ...
    ext_report_xml_dir = current_dir + '\\external_reports'
    ...
    # Returns Dictionary name_prod_dict having 'Product names' as KEY and the 'index' of Product start line as VALUE.
    # Also removes empty lines from the file data.
    ...
    # Returns name_cont_dict, cont_dict, name_cont_map_dict, self.fail_test_dict.
    # name_cont_dict - Dictionary having Containers name as KEY and respective Products as VALUE.
    # cont_dict - Dictionary having Containers value as KEY and respective Products as VALUE.
    # name_cont_map_dict - Dictionary having Containers name as KEY and respective Containers values as VALUE.
    # self.fail_test_dict - Dictionary having Failed Products as KEY and respective result log (Total passed and Failed) as VALUE.
    def dsl_file_container_chk(self):
        ...
        for product in self.name_prod_dict:
            ...
            for i in range(int(self.name_prod_dict[product]), len(self.file_data_list) - 1):
                ...
                if product.lower() in self.file_data_list[i + 1].lower() and '"Is-part of"' in self.file_data_list[
                    i + 1]:
                    if j != 0:
        ...
        print("self.name_prod_dict_end[product] +++++++ ", self.name_prod_dict_end[product])
        print("Combined containers list - ", self.cont_list_complete)
        print("Container mapping - ", name_cont_map_dict)
        print("Failed Test Dict ----------------- ", self.fail_test_dict)
        return name_cont_dict, cont_dict, name_cont_map_dict, self.fail_test_dict

    # Checks for report result in the reports under the external_reports folder.
    # And Updates the Class variable fail_test_dict with Failed Products as KEY
    # and respective result log (Total passed and Failed) as VALUE.
                    ...
                    for result in result_data_list:
                        if "Failures:" in result:
                            ...
                            if int(fail_test) > 0:
                                ...
                                self.fail_test_dict[container_value] = result
    ...
    def dsl_file_cont_relation(self):
        ...
        for product in self.name_prod_dict:
            ...
            for item in container_status_dict:
                if item in self.fail_test_dict:
            ...
            for i in range(len(self.cont_relation_dict)):
                for item in container_status_dict:
                    if item in self.cont_relation_dict.keys() and \
                            not container_status_dict[self.cont_relation_dict[item]]:
                        container_status_dict[item] = container_status_dict[self.cont_relation_dict[item]]

            for item in container_status_dict:
                if item not in self.fail_test_dict and not container_status_dict[item]:
                    container_status_dict[item] = "par_fail"
                elif container_status_dict[item]:
                    container_status_dict[item] = "pass"
                else:
                    container_status_dict[item] = "fail"
    ...
    # Checks for Environments under each Product, and there relation with the Containers.
    # Returns Dictionary depl_env_dict having Environment names as KEY and the products related to respective
    # environments in list as VALUE.
